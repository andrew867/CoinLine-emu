...
int main()
{
	std::filesystem::path const path = root() / "fixtures/display/vfd-11line-ad.json";
	...
	if (!v.text_rows_match_fixture_json(file) || v.raw_log() != bytes) {
		std::cerr << "11-line ad text/raw mismatch\n";
		return 1;
	}
	if (!v.milestone_m6_met()) {
		std::cerr << "expected M6-capable buffer state after ad fixture replay\n";
		return 1;
	}
	return 0;
}
