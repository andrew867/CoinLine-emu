...
int main()
{
	std::ifstream in(emu_root() / "fixtures/z180/mmu-registers.json");
	if (!in) {
		std::cerr << "missing mmu-registers fixture\n";
		return 1;
	}
	std::string const txt((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
	for (char const *key : { "\"CBR\"", "\"BBR\"", "\"CBAR\"" }) {
		if (txt.find(key) == std::string::npos) {
			std::cerr << "missing MMU key " << key << "\n";
			return 1;
		}
	}
	return 0;
